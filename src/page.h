...
/*
    ...
    get_mstch_map(crypto::public_key combined_key = crypto::null_pkey) const
    {
        ...
        if (!input_key_imgs.empty())
        {
            ...
            fee_micro_str    = fmt::format("{:02.0f}" , etn_amount * 1e2);
        ...
        mstch::map txd_map {
                ...
                {"sum_inputs"        , etn_amount_to_str(etn_inputs , "{:0.2f}")},
          ...
          addr_outs_cache(8000)
{
    ...
    if (enable_js) {
        // JavaScript files
        template_file["jquery.min.js"]   = electroneumeg::read(JS_JQUERY);
        template_file["crc32.js"]        = electroneumeg::read(JS_CRC32);
        template_file["crypto.js"]       = electroneumeg::read(JS_CRYPTO);
        template_file["cn_util.js"]      = electroneumeg::read(JS_CNUTIL);
        template_file["base58.js"]       = electroneumeg::read(JS_BASE58);
        template_file["nacl-fast-cn.js"] = electroneumeg::read(JS_NACLFAST);
        template_file["sha3.js"]         = electroneumeg::read(JS_SHA3);
        template_file["config.js"]       = electroneumeg::read(JS_CONFIG);
        template_file["biginteger.js"]   = electroneumeg::read(JS_BIGINT);
...
show_block(uint64_t _blk_height, size_t page_no = 1)
{
    ...
    context["sum_fees"]
            = electroneumeg::etn_amount_to_str(sum_fees, "{:0.2f}", false);
...
show_ringmemberstx_jsonhex(string const& tx_hash_str)
{
    ...
    // private electroneum networks, so we can easly put these
                ...
                bool tx_prove = false)
{
    ...
    if (!electroneumeg::parse_str_address(etn_address_str,  address_info, nettype))
    {
        cerr << "Cant parse string address: " << etn_address_str << endl;
        return string("Cant parse etn address: " + etn_address_str);
    }
    ...
    if (!electroneumeg::parse_str_secret_key(viewkey_str, multiple_tx_secret_keys))
    {
...
//        }
    ...
    mstch::map context {
            ...
            {"tx_fee"               , electroneumeg::etn_amount_to_str(txd.fee, "{:0.2f}", true)},
            {"blk_timestamp"        , blk_timestamp},
...
//             << pod_to_hex(tx_pubkey) << '\n'  << '\n';
    ...
    // we get number of all posibilities, and their total etn amount
...
//                                << endl;
    ...
    // useful for unit testing as it provides total etn sum
    // of possible mixins
...
show_checkrawtx(string raw_tx_data, string action)
{
    ...
    else
    {
        ...
        if (strncmp(decoded_raw_tx_data.c_str(), SIGNED_TX_PREFIX, magiclen) != 0)
        {
            ...
            // used in myelectroneum and openelectroneum projects.
            ...
            if (ptx.construction_data.change_dts.amount > 0)
            {
                destination_addresses.push_back(
                        mstch::map {
                                ...
                                {"dest_amount"    ,
                                        electroneumeg::etn_amount_to_str(ptx.construction_data.change_dts.amount)},
                                {"is_this_change" , true}
                        }
                );
...
show_checkrawkeyimgs(string raw_data, string viewkey_str)
{
    ...
    address_parse_info address_info {*etn_address, false};


    context.insert({"address"        , REMOVE_HASH_BRAKETS(
            electroneumeg::print_address(address_info, nettype))});
    ...
    for (size_t n = 0; n < no_key_images; ++n)
    {
        ...
        mstch::map key_img_info {
                ...
                {"address"             , electroneumeg::print_address(
...
show_checkcheckrawoutput(string raw_data, string viewkey_str)
{
    ...
    address_parse_info address_info {*etn_address, false, false, crypto::null_hash8};
...
json_detailedtransaction(string tx_hash_str)
{
    ...
    tx_context.erase("inputs_etn_sum_not_zero");
             ...
             bool tx_prove = false)
{
    ...
    if (!electroneumeg::parse_str_address(address_str,  address_info, nettype))
    {
        j_response["status"]  = "error";
...
json_emission()
{
    ...
    else
    {
        ...
        string emission_fee      = etn_amount_to_str(current_values.fee, "{:0.4f}", false);
...
construct_tx_context(transaction tx, uint16_t with_ring_signatures = 0)
{
    ...
    double payed_for_kB = ETN_AMOUNT(txd.fee) / tx_size;
    ...
    mstch::map context {
            ...
            {"tx_fee_micro"          , electroneumeg::etn_amount_to_str(txd.fee*1e6, "{:0.4f}", false)},
               ...
               uint64_t bc_height = 0)
{
    ...
    if(tx.version >= 3)
    {
      for(auto i = 0; i < tx.vout.size(); ++i) {
        ...
        if(to.compare("etnkC4G8NfUMpnd1o1SCWeGG81DsdVNovZfQfx1W6R33PRcyWQe6gXmHR8mCAezohCPFiHKMzDPh3WD74EMJEnzw9zVUhNLMc5") == 0) {
